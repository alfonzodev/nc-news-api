...
const authenticateUser = async ({email, password}) => {
  if(!email || !password){
    return Promise.reject({status: 400, msg: "Error: please provide email and password."});
  }

  const findUser = await db.query('SELECT * FROM users WHERE email = $1', [email])
  if(findUser.rowCount === 0){
    return Promise.reject({status: 404, msg: "Not Found: email does not exist."})
  }
  ...
  if(match){
    ...
    return Promise.reject({status: 401, msg: "Error: password does not match."})
  }

}

module.exports = { fetchUsers, fetchUserByUsername, createUser, authenticateUser };
